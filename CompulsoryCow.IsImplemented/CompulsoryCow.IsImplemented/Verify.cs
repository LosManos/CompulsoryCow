...
public class Verify
{
    ...
    public bool AreAllEqualsImplementedCorrectly([DisallowNull] Assembly assembly)
    {
        ...
        IEnumerable<(Type ClassType, bool IsImplementedCorrectly)>
            IsImplementedCorrectlyOrNot(IList<Type> classTypes) =>
            classTypes
                .Select(ct => (
    ...
    /// 
    /// If the class has a property of a type that is not handled by <see cref="IsImplemented"/>, 
    ...
    private static void SetPropertyToValue<T>(T o, PropertyInfo property, IDictionary<Type, object> values)
    {
        ...
        if (key == null)
        {
            throw new TypeNotRecognisedException(
                $"The type {property.PropertyType} has no value to test with. Use {nameof(SetComparisonValues)}");
        }
